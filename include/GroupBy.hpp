...
class GroupBy {
    ...
    class GroupIterator {
       public:
        ...
        constexpr Group operator*() const {
            const Key key =
                (m_groupBegin != m_groupEnd) ? m_groupBy.m_keyGetter(*m_groupBegin) : Key();
       ...
       private:
        ...
        friend constexpr bool operator==(const GroupIterator& lhs, const GroupEndIterator& rhs) {
            return lhs.m_groupBegin == rhs;
        }
    };

    /// Construct the group-by proxy for an iterator range.
    constexpr GroupBy(const Iterator& begin, const Iterator& end, KeyGetter keyGetter = KeyGetter())
        : m_begin(begin), m_end(end), m_keyGetter(std::move(keyGetter)) {}
    constexpr GroupIterator begin() const { return GroupIterator(*this, m_begin); }
   ...
   private:
    ...
    constexpr Iterator findEndOfGroup(const Iterator& start) const {
        ...
        return std::find_if_not(std::next(start), m_end, [this, start](const auto& x) {
...
auto makeGroupBy(Range&& range, KeyGetter keyGetter)
    ...
    return {std::ranges::begin(range), std::ranges::end(range), std::move(keyGetter)};
}
