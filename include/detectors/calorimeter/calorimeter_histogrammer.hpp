...
// calorimeter_histogrammer.hpp — Calorimeter validation histograms.
// Currently just per-event hit multiplicity; independent of the other
// subdetector histogrammers, so calorimeter-specific monitoring can grow
...
class CalorimeterHistogrammer {
   public:
    ...
    ~CalorimeterHistogrammer() {
        fill_contexts_.clear();
        file_service_->put("h_calorimeter_multiplicity", "Calorimeter hits per event;N;Events",
                           *h_multiplicity_);
    }
