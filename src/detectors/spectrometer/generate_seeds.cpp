...
class SeedGenObject {
 public:
  explicit SeedGenObject(std::shared_ptr<SpillContext> const& ctx) : ctx_{ctx} {
    if (!ctx_ || !ctx_->measurements) return;
    ...
    for (auto const& sl : ActsExamples::selectModule(ctx_->measurements->sourceLinks(), station1Id))
      hits1_.push_back(sl.index());
    for (auto const& sl : ActsExamples::selectModule(ctx_->measurements->sourceLinks(), station2Id))
  ...
  std::optional<std::size_t> find_next_valid(std::size_t from) const {
    if (hits1_.empty() || hits2_.empty()) return std::nullopt;
    ...
    for (std::size_t k = from; k < total; ++k) {
      ...
      if (dir.z() >= kMinForwardCosine) return k;
    }
    return std::nullopt;
  }
...
void register_generate_seeds(ModuleProxy& m, phlex::experimental::identifier const& layer,
  ...
  m.unfold<SeedGenObject>(
       "generate_seeds",
       [](SeedGenObject const&, std::optional<std::size_t> next) {
