...
// To-do - make this properly defined or automatically loaded from the field map. For now, we hardcode the bounds of the field map.
struct FieldBoundsM {
  double xmin = -4000* Acts::UnitConstants::mm, xmax = 4000* Acts::UnitConstants::mm, ymin = -4000* Acts::UnitConstants::mm, ymax = 4000* Acts::UnitConstants::mm, zmin = 0* Acts::UnitConstants::mm, zmax = 100000* Acts::UnitConstants::mm;
};
...
public:
  ...
  [[nodiscard]] Acts::Result<Acts::Vector3> getField(
      const Acts::Vector3& posActs) const {
    return doEvaluate(posActs);
  }

private:
  [[nodiscard]] Acts::Result<Acts::Vector3> doEvaluate(
      const Acts::Vector3& posActs) const {
    if (!m_eval) {
    ...
    // finite input guard
    if (!std::isfinite(posActs[0]) || !std::isfinite(posActs[1]) || !std::isfinite(posActs[2])) {
      return Acts::Result<Acts::Vector3>::success(Acts::Vector3::Zero());
    }
    ...
    if (x_mm < bounds.xmin || x_mm > bounds.xmax ||
      y_mm < bounds.ymin || y_mm > bounds.ymax ||
    ...
    return Acts::Result<Acts::Vector3>::success(
        Acts::Vector3{bx_T * Acts::UnitConstants::T,
