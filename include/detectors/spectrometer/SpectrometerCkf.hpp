...
selectNearestMeasurement(std::vector<TrackStateProxy>& candidates,
                         bool& isOutlier, const Acts::Logger&) {
  isOutlier = false;
  if (candidates.empty())
...
SHiP::TrackFitResult trackProxyToFitResult(TrackProxy const& track,
                                           TrackStateContainer& states,
  ...
  states.visitBackwards(track.tipIndex(), [&](auto ts) {
    if (!ts.typeFlags().isMeasurement()) return true;
    if (!ts.hasCalibrated()) return true;
    if (!ts.hasFiltered()) return true;
    ...
    if (!haveRefParams) {
      ...
      auto const global = ts.referenceSurface().localToGlobal(
          gctx, Acts::Vector2{filtered[Acts::eBoundLoc0], filtered[Acts::eBoundLoc1]}, direction);
...
class SpectrometerCkf {
 public:
  ...
  SpectrometerCkf(std::shared_ptr<const Acts::TrackingGeometry> geometry,
    ...
    // tightly to cover the ~2m station spacing within the step budget.
    {
      ...
      if (fieldRes.ok())
        std::cout << "[INFO]: field at " << samplePos.transpose() << " = "
                  ...
                  << (fieldRes.value() / Acts::UnitConstants::T).transpose()
                  << " T\n";
      else
    ...
    Navigator navigator(
        navCfg, Acts::getDefaultLogger("Navigator", Acts::Logging::INFO));

    Propagator propagator(std::move(stepper), std::move(navigator));
    m_ckf = std::make_unique<Ckf>(
        std::move(propagator),
    ...
    Navigator fitNavigator(
        fitNavCfg, Acts::getDefaultLogger("FitNavigator", Acts::Logging::INFO));
  ...
  void findTracks(Acts::BoundTrackParameters const& seed,
    ...
    trackStateCreator.sourceLinkAccessor
        .template connect<&IndexSourceLinkAccessor::range>(&sourceLinkAccessor);
    trackStateCreator.calibrator
        .template connect<&SpectrometerMeasurements::calibrator<Trajectory>>(
            &measurements);
    ...
    trackStateCreator.measurementSelector
        .template connect<&selectNearestMeasurement<
            typename TrackContainer::TrackStateProxy>>();
    ...
    Acts::CombinatorialKalmanFilterOptions<TrackContainer> options(
        m_geoContext, m_magContext, std::cref(m_calibContext), extensions,
        propOptions);
  ...
  std::vector<std::vector<ActsExamples::Index>> findCandidateHits(
    ...
    for (auto const& trackProxy : *result) {
      if (trackProxy.tipIndex() == Acts::kTrackIndexInvalid) continue;
      std::vector<ActsExamples::Index> hits;
      tracks.trackStateContainer().visitBackwards(
          trackProxy.tipIndex(), [&](auto ts) {
            if (!ts.typeFlags().isMeasurement()) return true;
            ...
            if (!ts.hasUncalibratedSourceLink()) return true;
            auto const& isl =
                ts.getUncalibratedSourceLink().template get<IndexSourceLink>();
            hits.push_back(isl.index());
            return true;
          });
      if (!hits.empty()) out.push_back(std::move(hits));
    }
    return out;
  }
  ...
  SHiP::TrackFitResult refit(Acts::BoundTrackParameters const& seed,
    ...
    ext.calibrator.template connect<&SpectrometerMeasurements::calibrator<Trajectory>>(
        &measurements);
    ext.updater.template connect<&Acts::GainMatrixUpdater::operator()<Trajectory>>(
        &kfUpdater);
    ...
    Acts::KalmanFitterOptions<Trajectory> options(
        m_geoContext, m_magContext, std::cref(m_calibContext), ext, propOptions,
    ...
    auto result =
        m_fitter->fit(sourceLinks.begin(), sourceLinks.end(), seed, options, tracks);
