...
// timing_detector_histogrammer.hpp — Timing Detector validation histograms.
// Currently just per-event hit multiplicity; independent of the other
// subdetector histogrammers, so timing-detector-specific monitoring can
// grow here without touching them.
...
class TimingDetectorHistogrammer {
   public:
    ...
    ~TimingDetectorHistogrammer() {
        fill_contexts_.clear();
        file_service_->put("h_timing_detector_multiplicity",
                           "Timing detector hits per event;N;Events", *h_multiplicity_);
    }
